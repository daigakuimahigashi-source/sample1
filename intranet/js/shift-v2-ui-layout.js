(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      /* --- employee master: one section at a time --- */
      #view-master .master-section-tabs{
      ...
      #view-master .master-section-tab{
        border:0;background:transparent;color:#667085;border-radius:8px;
        padding:8px 12px;font-size:10px;font-weight:900;white-space:nowrap;
      }
      ...
      #view-master .work-plan-table-wrap,
      #view-master .overtime-table-wrap,
      #view-master .overtime-master-table-wrap,
      #view-master [class*="overtime"][class*="table-wrap"]{
        max-height:none!important;
        overflow-y:visible!important;
        overflow-x:auto!important;
      }
      #view-master .master-table-card,
      #view-master #work-plan-panel,
      #view-master #overtime-master-panel{margin-bottom:10px}
      #view-master .master-section-pane[data-master-pane="employee"] .master-summary{margin-bottom:10px}
      #view-master .master-section-pane[data-master-pane="workplan"] #work-plan-panel,
      #view-master .master-section-pane[data-master-pane="overtime"] #overtime-master-panel{border-radius:12px}

      /* --- planner: give the gantt the width, inspector becomes a drawer --- */
      #view-planner .planner{
        grid-template-columns:220px minmax(0,1fr)!important;
        gap:10px!important;
      }
      #view-planner .gantt-card{min-width:0;width:100%}
      #view-planner .inspector-panel{
      ...
      #view-planner .inspector-panel .panel-title{
        position:sticky;top:0;z-index:2;background:#fff;
        display:flex;align-items:center;justify-content:space-between;gap:8px;
      }
      #view-planner .inspector-panel .panel-title>span:first-child{min-width:0}
      #view-planner .okk-inspector-close{
        border:1px solid #d0d5dd;background:#fff;color:#475467;border-radius:7px;
      ...
      #view-planner .okk-detail-hint{
        font-size:8px;color:#667085;font-weight:700;margin-left:4px;
      }
      @media(max-width:1120px){
        #view-planner .planner{grid-template-columns:200px minmax(0,1fr)!important}
        #view-planner .inspector-panel{grid-column:auto!important}
      }
      @media(max-width:760px){
        #view-planner .planner{display:block!important}
        #view-planner .inspector-panel{left:8px!important;right:8px!important;top:auto!important;bottom:8px!important;width:auto!important;max-height:72vh!important}
      }
    `;
  ...
  function setupMasterTabs() {
    ...
    if (!tabs) {
      ...
      tabs.innerHTML = `
        ...
        <button type="button" class="master-section-tab" data-master-section="workplan"><i class="fa-solid fa-id-badge"></i>正社員A/Bプラン</button>
    ...
    const workPlanPane = ensurePane(master, 'workplan');
    if (workPlan && workPlan.parentElement !== workPlanPane) workPlanPane.appendChild(workPlan);
    ...
    // Put all three panes directly after the tab bar, in a stable order.
    const anchor = tabs;
    [employeePane, workPlanPane, overtimePane].forEach((pane, index) => {
      const expectedPrevious = index === 0 ? anchor : [employeePane, workPlanPane][index - 1];
      if (pane.previousElementSibling !== expectedPrevious) expectedPrevious.insertAdjacentElement('afterend', pane);
    });

    const saved = sessionStorage.getItem(MASTER_TAB_KEY) || 'employee';
    activateMasterSection(saved, false);
  }
  ...
  function activateMasterSection(key, persist = true) {
    const master = document.getElementById('view-master');
    if (!master) return;
    const valid = ['employee', 'workplan', 'overtime'].includes(key) ? key : 'employee';
    ...
    master.querySelectorAll('.master-section-pane').forEach(pane => {
      pane.classList.toggle('active', pane.dataset.masterPane === valid);
    });
  ...
  function closeInspector() {
    document.querySelector('#view-planner .inspector-panel')?.classList.remove('okk-inspector-open');
  }
  ...
  function observeDynamicUi() {
    ...
    masterObserver.observe(workspace, { childList:true, subtree:true });
  }
  ...
  function bindGlobalEvents() {
    document.addEventListener('click', event => {
      ...
      if (shiftBar) {
        setTimeout(openInspector, 0);
        return;
      }

      const overtimeMaster = event.target.closest?.('#overtime-open-master');
      if (overtimeMaster) {
        setTimeout(() => activateMasterSection('overtime'), 30);
      }
    }, true);
    ...
    document.addEventListener('shiftv2-access', () => {
      setTimeout(() => {
        setupMasterTabs();
        setupPlannerLayout();
      }, 0);
