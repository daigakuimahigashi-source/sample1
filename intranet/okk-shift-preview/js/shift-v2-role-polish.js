(() => {
  ...
  document.addEventListener('click', event => {
    ...
    if (deleteButton) {
      ...
      if (record && !canType(access, record.type)) block(event, `${label(record.type)}の削除権限がありません`);
    }
  }, true);
  ...
  function canType(access, type) {
    const permission = type === 'absence' ? 'shift.exception.absence' : 'shift.exception.emergency_call';
  ...
  function label(type) { return type === 'absence' ? '欠勤' : '臨時招集'; }
